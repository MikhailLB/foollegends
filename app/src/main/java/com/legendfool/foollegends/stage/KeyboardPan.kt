 ...
 */
class KeyboardPan(private val host: View, private val box: StrongBox) {
    ...
    private val probe = Runnable {
        web?.evaluateJavascript("window.__flKbReport && window.__flKbReport();", null)
    }
            ...
            object : WindowInsetsAnimationCompat.Callback(DISPATCH_MODE_STOP) {
                ...
                override fun onStart(
                    ...
                    if (animation.typeMask and WindowInsetsCompat.Type.ime() != 0) {
                        ...
                        if (BuildConfig.DEBUG) Log.i(TAG, "ime heading for $declaredKb")
                    }
                    return bounds
                }
    ...
    private fun settle(height: Int) {
        ...
        settledKb = height
        box.rememberKeyboardRest(portrait(), height)
    }
    ...
    private fun resting(): Int {
        if (settledKb <= 0) {
            val kept = box.keyboardRest(portrait())
    ...
    private inner class Bridge {
         ...
         */
        @JavascriptInterface
        fun focus(frame: Boolean, top: Double, bottom: Double, note: String) {
            val view = web ?: return
            view.post {
                framed = frame
                ...
                if (BuildConfig.DEBUG) {
                    Log.i(
                        TAG,
                        "field ${top.toInt()}..${bottom.toInt()} frame=$frame ime=$keyboard " +
                            "span=${view.height} riding=$riding pan=${-offset(view)} | $note"
                    )
                }
    ...
    val script: String = """
        (function(){
          ...
          function report(){
            ...
            var note = 'inner=' + window.innerHeight +
                       ' vv=' + (vv ? Math.round(vv.height) : -1) +
                       ' off=' + Math.round(lift) +
                       ' dpr=' + window.devicePixelRatio + ' zoom=' + zoom;
            try {
              $BRIDGE.focus(
                at.frame,
                (at.top - lift) * px,
          ...
          function watch(doc){
            try {
              if (!doc || doc.__flKb) return;
              doc.__flKb = true;
              doc.addEventListener('focusin', kick, true);
            } catch(e) {}
          }

          window.__flKbReport = report;
          watch(document);
        })();
    """.trimIndent()
    ...
    private companion object {
        const val TAG = "FLKB"
        const val BRIDGE = "FLKeyboardPan"
